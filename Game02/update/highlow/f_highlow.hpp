...
void HL_disp_message(Texture& sys, Texture& frm, bool& update, bool g_set, short& chain) {
  ...
  if (g_set) {
    ...
    // ハイスコア更新の表示
    if (update) {
      x = (-Window::WIDTH / 2) + Icon_W * 4;
      y = (-Window::HEIGHT / 2) + (Telop_H * 3.5);

      // 「UPDATE !」
      drawTextureBox(x + (Icon_W / 2), y + (Telop_H / 2), Icon_W, Telop_H,
        Icon_W * 2, Telop_H * 2, Icon_W, Telop_H,
        sys, Color(1, 1, 0.5),
        0, Vec2f(1 + (abs(sin(angle2)) * 0.2), 1 + (abs(sin(angle2))) * 0.2), Vec2f(Icon_W / 2, Telop_H / 2));
    }
  }
}
